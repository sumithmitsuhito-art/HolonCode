...
class AIService:
    ...
    def initialization(self):
        ...
        self.difficulty = learning.get("Difficulty", "medium")
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=60) as client:
                while True:
                    system_msg = Message(
                        role="system",
                        content=self.prompt_manager.build_system_prompt(
                            self.active_skills, self.difficulty
                        ),
                    )
                    ...
                    if finish_reason == "stop":
                        ...
                        await self.tool.cleanup_plugins()
                        return
                        ...
                        for tc in tool_calls:
                            ...
                            try:
                                result = await self.tool.tool_actor_async(
                                    name,
    ...
    def set_difficulty(self, difficulty: str) -> None:
        ...
        if difficulty in valid:
            ...
            # 同步写入配置文件，持久化保存
            self._save_difficulty(difficulty)

    def _save_difficulty(self, difficulty: str) -> None:
        config_path = DATA_DIR / "UserSettings.json"
        try:
            config = json.loads(config_path.read_text(encoding="utf-8"))
            if "Learning" not in config:
                config["Learning"] = {}
            config["Learning"]["Difficulty"] = difficulty
