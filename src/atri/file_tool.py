...
# Windows reserved device names (case-insensitive). Opening these as files
# can hang the process or interact with hardware devices.
...
def _has_unsafe_component(user_path: str) -> bool:
    """Check every path component for Windows reserved names or NTFS ADS."""
    for part in user_path.replace('\\', '/').split('/'):
        ...
        if ':' in stripped:
            return True
        # Windows reserved device name (CON, NUL, etc.)
        basename = stripped.rsplit('.', 1)[0]
        if _WIN_RESERVED.match(basename):
            return True
    return False


def _block_c_drive(path: Path) -> str | None:
    """Block deletion of any file residing on the C: drive. Returns error string if blocked."""
    try:
        drive = path.resolve().drive
    except OSError:
        return f"无法解析路径盘符：{path}"
    if drive.upper() == "C:":
        return "安全拦截：禁止删除 C 盘文件。请手动在回收站中处理。"
    return None
...
class FileTool:
    ...
    def get_safe_path(cls, user_path: str) -> Path | None:
        ...
        if ".." in stripped:
            return None
        if _has_unsafe_component(stripped):
            return None
        full = (Path(cls.work_dir) / stripped).resolve()
    ...
    def write_file(cls, file_path: str, content: str) -> str:
        ...
        if len(content.encode("utf-8")) > cls.max_file_size:
            return f"文件大小超过上限（{cls.max_file_size // 1024 // 1024}MB），请精简内容或分拆到多个文件。"
        try:
    ...
    def append_file(cls, file_path: str, content: str) -> str:
        ...
        if safe is None:
            return "安全拦截：不允许访问工作目录以外的路径。"
        existing = safe.stat().st_size if safe.is_file() else 0
        added = len(content.encode("utf-8"))
        if existing + added > cls.max_file_size:
            return f"追加后文件将超过上限（{cls.max_file_size // 1024 // 1024}MB），当前 {existing} + 新增 {added} 字节。"
        try:
            safe.parent.mkdir(parents=True, exist_ok=True)
    ...
    def delete_file(cls, file_path: str, confirm: str) -> str:
        ...
        c_block = _block_c_drive(safe)
        if c_block:
            return c_block
        try:
            send2trash.send2trash(str(safe))
            return f"已成功将文件移入回收站：{file_path}"
        except Exception as e:
            return f"删除文件失败：{e}"
