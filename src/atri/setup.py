...
def configure_api() -> None:
    ...
    api_model = input("  DeepSeek Model (默认 deepseek-v4-pro): ").strip()
    ...
    settings = {
        "DeepSeek": {
            ...
            "Model": api_model or "deepseek-v4-pro",
        }
    }
...
def configure_qqbot() -> None:
    """Configure optional QQ Bot credentials."""
    _header("第五步：QQ Bot 配置（可选）")
    ...
    if config_path.exists():
        try:
            data = json.loads(config_path.read_text(encoding="utf-8"))
            current_qq = data.get("QQBot", {})
        except (json.JSONDecodeError, OSError):
            pass

    print("  QQ Bot 功能允许你通过 QQ 与小洛聊天。")
    print("  需要前往 QQ 开放平台 (https://q.qq.com) 创建机器人应用获取凭据。")
    print()

    if current_qq:
        app_id_preview = current_qq.get("AppId", "")[:8] + "****" if current_qq.get("AppId") else "(未设置)"
        print(f"  当前 AppId:        {app_id_preview}")
        print(f"  当前 ClientSecret:  {'已设置' if current_qq.get('ClientSecret') else '(未设置)'}")
        print()
        if input("  是否修改？(y/N) ").strip().lower() != "y":
            print("  保持现有 QQ Bot 配置不变。")
            return

    app_id = input("  QQ Bot AppId (留空跳过): ").strip()
    client_secret = input("  QQ Bot ClientSecret (留空跳过): ").strip()

    if not app_id and not client_secret:
        print("  未配置 QQ Bot，可在稍后配置。")
        return
