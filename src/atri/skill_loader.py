...
class SkillLoader:
    ...
    def write_skill(name: str, content: str) -> bool:
        ...
        skill_dir = SkillLoader._safe_skill_path(name)
    ...
    def _safe_skill_path(name: str) -> Path | None:
        """Resolve a skill sub-path and verify it stays inside SKILLS_DIR. Returns None if unsafe."""
    ...
    @staticmethod
    def load_plugin(name: str, ai_service=None):
        """Dynamically load a skill's plugin.py module. Returns module or None.

        If ai_service is provided, it is injected into the plugin namespace
        so tools can call ai_service.set_difficulty() etc.
        """
        ...
        try:
            ...
            module.tool = tool
            # 注入 AIService 引用，便于插件工具调整全局状态
            if ai_service is not None:
                module.ai_service = ai_service
