...
_SESSIONS_DIR_RESOLVED = _SESSIONS_DIR.resolve()


def _is_safe_session_path(path: Path) -> bool:
    """Verify that *path* (resolved) stays within the sessions directory."""
    try:
        path.relative_to(_SESSIONS_DIR_RESOLVED)
        return True
    except ValueError:
        return False
...
class ConversationManager:
        ...
        try:
            ...
            for sid, messages in all_data.items():
                ...
                if not _SESSION_ID_RE.match(sid_str):
                    continue
                dst = _SESSIONS_DIR / f"{sid_str}.json"
                if not dst.exists():
                    dst.write_text(
    ...
    def delete_session_history(session_id: str):
        if not _SESSION_ID_RE.match(session_id):
            return
        p = (_SESSIONS_DIR / f"{session_id}.json").resolve()
        if not _is_safe_session_path(p):
            return
        if p.exists():
            p.unlink()
