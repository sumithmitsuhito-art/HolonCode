...
from atri.skill_loader import SkillLoader, extract_tools, MAX_ACTIVE_SKILLS
...
class ToolManager:
    ...
    def set_ai_service(self, ai_service):
        """注入 AIService 引用，供 skill plugins 调整全局状态。"""
        self._ai_service = ai_service

    _ALL_BUILTIN_TOOLS: list[Tool] = [
        ...
        _make_tool("activate_skill",
            "激活一个技能，将其完整的行为指令注入系统提示词。激活后 小洛 的行为会在下一轮对话中改变。"
        ...
        _make_tool("write_skill_file",
            "创建或覆盖一个技能的 SKILL.md 文件。这是 小洛 自我迭代的核心工具——你可以通过此工具创建新技能或修改已有技能。"
    ...
    def tool_init(self):
        if not self.tool_list:
            self.tool_list.extend(self._ALL_BUILTIN_TOOLS)
            self._base_tools = list(self._ALL_BUILTIN_TOOLS)
    ...
    def tool_actor(self, name: str | None, arguments: str, active_skills: list[str] | None = None) -> str | None:
        ...
        if name == "activate_skill":
            ...
            # 加载 plugin.py（注入 AIService 引用供插件工具调整全局状态）
            plugin = SkillLoader.load_plugin(skill_name, self._ai_service)
            tool_names = []
            ...
            result = f"[已激活] {skill_name}  当前激活: {skills_list}"
            if tool_names:
    ...
    async def cleanup_plugins(self):
        """Close async resources held by active skill plugins (e.g. browser)."""
        for plugin in list(self._skill_plugins.values()):
            close_fn = getattr(plugin, "_close_browser", None)
    ...
    async def tool_actor_async(self, name: str | None, arguments: str, active_skills: list[str] | None = None) -> str | None:
        """Async variant: handles built-in sync tools + async skill tools."""
        if not name:
            return "未知工具调用"
        try:
            args = json.loads(arguments) if arguments and arguments.strip() else {}
        ...
        # Built-in tools are all sync — delegate to tool_actor
        builtin_names = {t.function.name for t in self._ALL_BUILTIN_TOOLS}
        if name in builtin_names:
            return self.tool_actor(name, arguments, active_skills)

        # Skill tools: check if async
        for skill_name in (active_skills or []):
            plugin = self._skill_plugins.get(skill_name)
            if plugin is None:
                continue
            func = getattr(plugin, name, None)
            if func is None or not callable(func) or not getattr(func, "_is_tool", False):
                continue
            try:
