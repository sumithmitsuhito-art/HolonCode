...
def _get_icon_data_uri() -> str:
    ...
    pix = QPixmap(str(RESOURCE_DIR / "icon" / "头像.png"))
    pix = pix.scaled(
...
_SKILL_MSG = (
    ...
    '<span style="display:inline-block;background:#E8F5E9;color:{fg};'
...
class Thread(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        self.setStyleSheet(
            f"#thread {{ background-color: {BG_CHAT}; }}"
        )
        ...
        self._kb_sourced = False  # 标记是否使用了知识库
    ...
    def mark_kb_sourced(self):
        self._kb_sourced = True
        if self._messages and self._messages[-1][0] == "ai-progress":
            self._streaming_pos = None
            self._full_render()
        ...
        if msg_type == "user":
            ...
            if self._web_sourced:
                content_html = (
                    ...
                    'border:1px solid #BBDEFB;margin-right:6px;">'
                    "\U0001f310 回答来自网络搜索"
                    '</span></div>'
                ) + content_html
