...
class FilePreviewDialog(QDialog):
    ...
    def __init__(self, file_path: str, parent=None):
        ...
        work = Path(FileTool.work_dir).resolve()
        try:
            resolved.relative_to(work)
        except ValueError:
            raise ValueError(f"路径不在工作目录内，已拦截: {file_path}")
    ...
    def _on_submit(self):
        """Save the file and emit submit_requested so AppShell can send it to AI."""
        try:
            Path(self._file_path).write_text(
                self._editor.toPlainText(), encoding="utf-8"
            )
            ...
            pass
        self.submit_requested.emit(self._file_path)
        self.accept()
...
class FilePanel(QFrame):
    ...
    file_submit_requested = Signal(str)
    ...
    def _on_double_click(self, index):
        ...
        if p.is_file():
            ...
            dlg.submit_requested.connect(self.file_submit_requested.emit)
            dlg.exec()
