...
class AppShell(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("DSParkCode")
        ...
        self._tools_called: set[str] = set()  # 追踪当前对话轮次调用的工具
        ...
        self.sidebar.c_qa_review_selected.connect(self._on_c_qa_review)
        ...
        # Check API key — show guidance if not configured
        if not self.ai_service.api_key:
            self.chat_view.thread.add_system_message(
                "欢迎使用 DSParkCode！\n\n"
                "请先配置 DeepSeek API Key 以开始使用：\n"
                "1. 点击右上角「设置」按钮\n"
                "2. 填入你的 DeepSeek ApiKey\n"
                "3. 点击「保存」\n\n"
                "还没有 API Key？请前往 platform.deepseek.com 注册获取。\n\n"
                "已配置好密钥？可能文件未找到，请检查 data/UserSettings.json。"
            )
    ...
    def _on_skill_activated(self, skill_name: str):
        ...
        if skill_name == "c-tutor":
            ...
            self.chat_view.submit_text("我们来角色互换复习吧～你来当小老师，我来提问，准备好了吗？")
    ...
    def _on_c_qa_review(self, module_id: str, module_name: str):
        """Activate c-qa skill with specific module selected for role-reversal review."""
        from atri.skill_loader import SkillLoader
        if not SkillLoader.skill_exists("c-qa"):
            return
        result = self.ai_service.tool.tool_actor(
            "activate_skill",
            '{"name":"c-qa"}',
        ...
        self.chat_view.thread.add_skill_message(result)
        self.status_bar.set_status("技能已激活: c-qa (角色互换复习)")
        self._update_status()
        # 发送指令，让小洛以角色互换模式针对指定模块提问
        prompt = (
            f"【角色互换模式】我选择了知识点模块：{module_id} — {module_name}\n"
            "你现在是小洛，用角色互换的方式来帮我复习这个模块。\n"
            "请你：\n"
            "1. 用温柔求教的语气向我提问（假装对这个知识点记得不太清楚）\n"
            "2. 围绕这个模块的核心概念提几个关键问题\n"
            "3. 通过这种方式引导我回顾这个模块的知识点\n"
            "4. 每次只问1个问题，循序渐进\n"
            "\n"
            "现在开始吧～"
        )
        self.chat_view.submit_text(prompt)
    ...
    def _dispatch_to_ai(self, text: str):
        ...
        if not self.ai_service.api_key:
            self.chat_view.thread.add_system_message(
                "请先点击右上角「设置」按钮配置 DeepSeek API Key，然后再发送消息。"
            )
            return
        ...
        self._current_ai_text = ""
        self._tools_called.clear()  # 重置工具调用追踪
    ...
    def _on_tool_start(self, tool_name: str):
        ...
        self._tools_called.add(tool_name)  # 记录调用的工具
        if tool_name in ("web_search", "web_extract"):
            self.chat_view.thread.mark_web_sourced()
        if tool_name == "c_knowledge_search":
            self.chat_view.thread.mark_kb_sourced()
    ...
    def _on_open_settings(self):
        ...
        if dlg.exec():
            # Reload API config so key changes take effect without restart
            self.ai_service.initialization()
