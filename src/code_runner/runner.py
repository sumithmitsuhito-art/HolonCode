...
def _safe_resolve(file_path: str) -> Path | None:
    """Resolve a path (absolute or relative) and verify it's inside the workspace."""
...
class CodeRunner:
    ...
    def needs_input(file_path: str) -> bool:
        ...
        if safe is None:
            return False
        lang = _LANG_MAP.get(safe.suffix, "")
    ...
    def run(self, file_path: str,
        ...
        safe = _safe_resolve(file_path)
        if safe is None:
            return "安全拦截：不允许运行工作目录以外的文件。"
        code = safe.read_text(encoding="utf-8")
        lang = self._detect_lang(str(safe))
